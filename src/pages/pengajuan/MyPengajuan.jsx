...
import {
  ...
  Eye, Clock, CheckCircle, XCircle, LayoutGrid
} from "lucide-react";
...
export default function MyPengajuan() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [filterStatus, setFilterStatus] = useState(""); // "" (all), atau status spesifik
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const myData = res.data.filter(item => item.user_id === userId);
      setData(myData);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <MainLayout>
      <div className="space-y-6 pb-10">
        ...
        {/* MINIMALIST INTERACTIVE STAT CARDS */}
        ...
        <div className="relative">
          ...
          <input
            type="text" placeholder="Cari nomor pengajuan atau keperluan..." value={search}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
          {loading ? (
            <div className="p-8"><TableSkeleton columns={6} rows={5} /></div>
          ) : (
                    ...
                    <tr>
                      <td colSpan={7} className="py-24 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Tidak ada pengajuan {filterStatus ? `dengan status ${STATUS_CONFIG[filterStatus]?.label}` : "ditemukan"}</p>
                          {filterStatus && (
                            <button onClick={() => setFilterStatus("")} className="text-[10px] font-black text-blue-600 uppercase tracking-widest hover:underline mt-2">Lihat Semua Pengajuan</button>
                          )}
                        </div>
                      </td>
                    </tr>
          ...
          {totalPages > 1 && (
            <div className="flex justify-between items-center px-8 py-6 border-t border-slate-50 dark:border-slate-800 bg-slate-50/10 dark:bg-slate-800/10">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                {indexFirst + 1}â€“{Math.min(indexLast, filtered.length)} dari {filtered.length} Berkas {filterStatus ? `(${STATUS_CONFIG[filterStatus]?.label})` : ""}
              </span>
