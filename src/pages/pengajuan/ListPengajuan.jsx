...
export default function ListPengajuan() {
  ...
  const stats = [
    ...
    { key: "pending_asisten_manager", label: "Menunggu Persetujuan Asmen", color: "from-amber-500 to-orange-500", icon: <ShieldCheck size={16} />, count: countByStatus("pending_asisten_manager") },
    { key: "pending_manager", label: "Menunggu Persetujuan Manager", color: "from-blue-600 to-sky-500", icon: <User size={16} />, count: countByStatus("pending_manager") },
    { key: "pending_gudang", label: "Menunggu Persetujuan Gudang", color: "from-indigo-600 to-purple-500", icon: <Package size={16} />, count: countByStatus("pending_gudang") },
    { key: "completed", label: "Pengajuan Selesai", color: "from-emerald-500 to-teal-600", icon: <CheckCircle size={16} />, count: countByStatus("completed") },
    { key: "rejected", label: "Pengajuan Ditolak", color: "from-rose-500 to-pink-600", icon: <XCircle size={16} />, count: countByStatus("rejected") },
  ];
    ...
    <MainLayout>
      <div className="space-y-6 pb-10">
        ...
        <PageHeader
          ...
          subtitle="Pantau dan kelola seluruh alur permintaan barang dalam sistem"
          actions={
          ...
          {totalPages > 1 && (
            <div className="flex justify-between items-center px-8 py-6 border-t border-slate-50 dark:border-slate-800 bg-slate-50/10 dark:bg-slate-800/10">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                Menampilkan {indexFirst + 1}â€“{Math.min(indexLast, filtered.length)} dari {filtered.length} Berkas
              </span>
